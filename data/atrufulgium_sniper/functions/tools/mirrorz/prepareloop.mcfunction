#Summon an armor stand at either side of the mirror axis: one at (x1,y1,z1) and one at (x1,y1,z2).
#Assumes the caller's selectionbox.
tellraw @s {"text": "§b[Atru Sniper] §fMirroring the selection along the z-axis"}
...
execute as @e[scores={interest=2}] run scoreboard players remove @s selectionbox_z2 1
...
execute as @e[scores={interest=1}] store result entity @s Pos[2] double 1 run scoreboard players operation @s posz = @s selectionbox_z1
execute as @e[scores={interest=2}] store result entity @s Pos[2] double 1 run scoreboard players operation @s posz = @s selectionbox_z2
execute as @e[scores={interest=1}] at @s run function atrufulgium_sniper:tools/mirrorz/loopthroughselection