#Summon an armor stand at either side of the mirror axis: one at (x1,y1,z1) and one at (x1,y2,z1).
#Assumes the caller's selectionbox.
tellraw @s {"text": "§b[Atru Sniper] §fMirroring the selection along the y-axis"}
...
execute as @e[scores={interest=2}] run scoreboard players remove @s selectionbox_y2 1
...
execute as @e[scores={interest=1}] at @s run function atrufulgium_sniper:tools/mirrory/loopthroughselection